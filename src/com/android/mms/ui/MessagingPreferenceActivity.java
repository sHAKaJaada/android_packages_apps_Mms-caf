...
public class MessagingPreferenceActivity extends PreferenceActivity
    ...
    private PreferenceCategory mManageSubsCategory;
    ...
    // Manage multi-sim preferences
    MSimPreferencesController mMSimPreferencesController;
    SmscBroadcastReceiver mSmscReceiver;

    @Override
    protected void onCreate(Bundle icicle) {
        super.onCreate(icicle);

        mMSimPreferencesController =
                new MSimPreferencesController(this, getPreferenceManager(),
                        MSimConstants.INVALID_SUBSCRIPTION);
        mMSimPreferencesController.setPreferenceHolder(this);
    ...
    protected void onStart() {
        ...
        if (!MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
            ...
            mMSimPreferencesController.refreshSmsc();
        }
    }
    ...
    private void updateSmsEnabledState() {
        ...
        mManageSubsCategory.setEnabled(mIsSmsEnabled);
    }

    private void setMultiSimPreferences() {
        PreferenceScreen prefRoot = (PreferenceScreen)findPreference("pref_key_root");
        if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
            // If multi-sim is enabled then remove msim properties from this preference
            // screen.  They will be shown under "subscription settings".
            mMSimPreferencesController.removePreferences();
        } else {
            Preference subSettingsCategory = findPreference("pref_key_manage_subs");
            prefRoot.removePreference(subSettingsCategory);
            mMSimPreferencesController.setupPreferences();
        }
    }
    ...
    private void loadPrefs() {
        ...
        mManageSubsCategory = (PreferenceCategory)findPreference("pref_key_manage_subs");
